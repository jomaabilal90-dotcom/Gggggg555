...
# قاموس بسيط للسياق بدلاً من cachetools
user_contexts = {}
...
def handle_message(message):
    try:
        ...
        chat = user_contexts.get(user_id)
        if not chat:
        ...
        if response and response.text:
            ...
            bot.reply_to(message, "عذراً، لم أتمكن من الحصول على رد. الرجاء المحاولة مرة أخرى.")
...
def setup_bot_commands():
    ...
    # إضافة أمر stats فقط للمشرف
    if ADMIN_ID:
        commands.append(BotCommand("stats", "Bot statistics (admin only)"))
    
    bot.set_my_commands(commands)

# ==================== تنظيف الذاكرة المؤقتة ====================
def cleanup_old_contexts():
    """تنظيف السياقات القديمة كل ساعة"""
    while True:
        time.sleep(3600)  # كل ساعة
        # تنظيف بسيط - في الإصدار الحقيقي نحتاج لوقت انتهاء الصلاحية
        if len(user_contexts) > 1000:  # إذا كان العدد كبيراً
            user_contexts.clear()
            logger.info("Cleaned up old contexts")

# بدء تنظيف الذاكرة
cleanup_thread = threading.Thread(target=cleanup_old_contexts, daemon=True)
